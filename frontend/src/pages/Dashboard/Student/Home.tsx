...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      // 1. Fetch Student's specific courses (based on user.courses array)
      const courseRes = await fetch(`http://localhost:5000/api/users/courses?program=${user.program}`, { headers });
  ...
  return (
    <div className="p-8 bg-[#F8F9FA] min-h-screen">
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <StatCard label="Current Semester" value={user?.semester || "N/A"} icon={<LuBookOpen className="text-orange-500"/>} />
        <StatCard label="Registered Courses" value={user?.courses?.length.toString() || "0"} icon={<LuGraduationCap className="text-green-500"/>} />
        <StatCard label="Current CGPA" value={user?.cgpa?.toFixed(2) || "0.00"} icon={<LuChartLine className="text-red-500"/>} />
        <StatCard label="Fee Status" 
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-4 space-y-8">
           ...
           <section className="bg-white p-6 rounded-[.5rem] border border-gray-100 shadow-sm">
             <h2 className="font-bold text-gray-800 mb-4 tracking-tight">Notifications</h2>
             <div className="space-y-4">
