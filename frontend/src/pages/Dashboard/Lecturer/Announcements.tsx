...
const AnnouncementsPage = () => {
  ...
  const fetchAnnouncements = async () => {
    const token = localStorage.getItem("token");
      const res = await fetch("http://localhost:5000/api/users/my-announcements", {
  ...
  const handleDelete = async (id: string) => {
    ...
    const res = await fetch(`http://localhost:5000/api/users/announcements/${id}`, {
      method: "DELETE",
...
const AddAnncouncement = ({ isOpen, onClose, onSuccess }: any) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
          const token = localStorage.getItem("token");
          const response = await fetch("http://localhost:5000/api/users/announcements", {
              method: "POST",
              headers: {
